...
/**
 * OutputThread writes data to a peer, and never reads in any data in order to prevent blocking and waiting, or some terrible constant 
 * back-and-forth keepalive. Data written isn't saved anywhere. This behavior doesn't need to be run in a thread, but would block activity
 * on the main thread if write calls were direct. Instead, any calls to OutputThread's external methods are extremely lightweight 
 * (putting a String in a buffer) rather than waiting on network IO to execute. 
 */
public class OutputThread extends Thread
{
    ...
    /**
     * Technically not writing to the network socket, but instead putting the passed-in data in a buffer
     * to be written to the socket as soon as possible.
     * 
    ...
    /**
     * Stops thread during the next write cycle.
     */
